...
xx=[]
for i in data:    
    res = cv2.resize(i , dsize=(112, 112), interpolation=cv2.INTER_CUBIC)
    xx.append(res)
xx=np.array(xx)
...
def define_discriminator(in_shape=(112,112,1)):
	model = Sequential()
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=150, n_batch=2):
